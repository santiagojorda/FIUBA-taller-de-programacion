...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        skt_aux = socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
        if (skt_aux == -1) {
            continue;
        }
         ...
         * */
        s = connect(skt_aux, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(skt_aux, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(skt_aux, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
