...
# ----
#
#   ./run_tests.sh <executables-folder> <cases-folder> <testing-model> <valgrind-mode> [timeout [kill_timeout]] [suppressions] [sleep] [pretty-marker]
#
# donde:
...
if [ "$#" = "0" ]; then
    echo "Usage:"
    echo
    echo "$0 <executables-folder> <cases-folder> <testing-model> <valgrind-mode> [timeout [kill_timeout]] [suppressions] [sleep-between-connects]"
    echo
    exit 1
fi
...
G_KILL_TIMEOUT=${6:-10}
suppressions_file=$7
G_SLEEP_BETWEEN_CONNECTS=${8:-6}
G_IN_SERCOM=${9:-0}

# Marker definition
PRETTYPRINTMARKER=${10:-"-------------------------"}
...
assert_program_is_in_the_path logger

hash hd 2>/dev/null
if [ "$?" != "0" ]; then
    HEXDUMP_PROG="hexdump -C"
else
    HEXDUMP_PROG="hd"
fi
...
run_bash_with_timeout () {
    ...
    timeout --kill-after $G_KILL_TIMEOUT $G_TIMEOUT bash "$1"
}
...
create_link_to_executable () {
    ...
    if [ "$program_name" = "$program_path" ]; then
        ...
        ret=$?
        if [ "$ret" != "0" ]; then
            echo "create_link_to_executable fallo con codigo de error $ret"
            abort_tests
        fi
    fi
}
...
spawn_server () {
    ...
    if [ "$G_IN_SERCOM" = "0" ]; then
        timeout --kill-after $G_KILL_TIMEOUT $G_TIMEOUT $server_cmd < $input_file >__server_stdout__ 2> >(logger --tag="server  " --priority "0.0" --rfc5424=notime,nohost --no-act --stderr)
    else
...
spawn_tiburoncin_and_client () {
    ...
    if [ "$G_IN_SERCOM" = "0" ]; then
        timeout --kill-after $G_KILL_TIMEOUT $G_TIMEOUT $client_cmd <$client_input >"__client_${N}_stdout__" 2> >(logger --tag="client-${N}" --priority "0.0" --rfc5424=notime,nohost --no-act --stderr) &
    else
    ...
    rm -f "__client.$N.args"

    # We ensure that the client is fully connected to the server so if the client and/or
    # server do something when the connection is established, this will happen for *this*
    # client first, and the for the *next* client (basically if run_tests.sh spawn the
    # clients too fast there is a RC between the clients to connect to the server and
    # it will make the test execution non-deterministic)
    sleep $G_SLEEP_BETWEEN_CONNECTS
}
...
for case_folder in $G_CASES_FOLDER/*; do
    ...
    # Clean temporal files that may exists if the run_tests.sh is executed
    # twice over the same folder.
    rm -f q.pipe __client_descriptors__ __client_*_pipe__ __client.*.args __client_*_pipe__
    rm -f __client_*_stdout__ __client_*_stderr__
    rm -f __server_stdout__ __server_stderr__
    ...
    if [ "$clients_pipe" == "yes" ]; then
        ...
        run_bash_with_timeout "clients_schedule.sh"
        clients_schedule_ret=$?
        abort_tests_if_ret_code_is_timeout $clients_schedule_ret "clients_schedule.sh" "Nota: es posible que alguno de los clientes se haya bloqueado/muerto y eso hizo que clients_schedule.sh se bloqueara."
    fi
    ...
    if [ "$G_VALGRIND_MODE" != "valgrind" ]; then
        if [ -s "__server_stderr__" ]; then
            ...
            cat "__server_stderr__" | $HEXDUMP_PROG
            echo
        fi
        ...
        for client_input in __client_*_args__; do
            ...
            if [ -s "__client_${clientn}_stderr__" ]; then
                ...
                cat "__client_${clientn}_stderr__" | $HEXDUMP_PROG
                echo
            fi
        done
    fi
